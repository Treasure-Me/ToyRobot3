...
class RobotTest {
    ...
    private void verifyOutput(String[] actualOutput, String[] expectedOutput) {
        int j = expectedOutput.length - 1;
        for (int i = actualOutput.length - 1; j >= 0; i--, j--) {
            assertEquals(expectedOutput[j].trim(), actualOutput[i].trim(),
                    "Expected line '" + expectedOutput[j] + "' but got '" + actualOutput[i] + "'");
        }
    }
